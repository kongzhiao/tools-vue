...
interface ImportResult {
  ...
  data?: {
    ...
    street_town_success_count:number;
    assistance_success_count: number;
    fail_count: number;
    total_rows: number;
    errors: string[];
  };
...
const ImportsTreetTowMatchModal: React.FC<ImportStreetTownMatchModalProps> = ({
  ...
  const handleImportYear = async () => {
    ...
    try {
      ...
      if (response.code === 0) {
        message.success(`导入完成，成功${response.data.street_town_success_count}条，跳过${response.data.fail_count}条`);
        // 立即刷新数据
        onSuccess();
      } else {
          ...
          description={
            <div>
              ...
              <ul style={{ marginBottom: 16 }}>
                ...
                <li><Text strong>认定区</Text>：参保人员的认定区</li>
                <li><Text strong>资助身份</Text>：参保人员的资助身份</li>
              </ul>
              <Paragraph style={{ marginBottom: 8 }}>
                2. 匹配成功的数据将自动更新认定区匹配状态，并将匹配状态设置为已匹配
              </Paragraph>
            </div>
          }
          type="info"
